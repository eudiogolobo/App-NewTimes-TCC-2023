...
if AltDelTurma=1 then
  ...
  begin
  AltDelMatricula:=4;
  IDTURMAS1:=dm.qturmasid_turmas.Value;
...
procedure Tfrm_pqaltturma.DBGrid1MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
...
begin
 with dm.qturmas do
           ...
           close;
            ...
            sql.Add('where a.CursoCod = b.CursosId ');
            open;
...
procedure Tfrm_pqaltturma.Panel1MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
 frm_prncipal.pnl_matricula.visible:=false;
 frm_prncipal.pnl_perfil.visible:=false;
...
begin
           ...
           close;
            ...
            sql.Add('select a.id_turmas, a.CursoCod, a.Turma,a.turno, a.dataInicio, a.DataFinal, a.status, b.CursosId, b.Nome ');
            sql.Add('from turmas as a, cursos as b');
            sql.Add('where a.CursoCod = b.CursosId and  a.Turma like ''%'+txt_buscar.Text+'%''');
            open;
            lb_total.Caption:=inttostr(dm.tbturma.RecordCount);
           end;
