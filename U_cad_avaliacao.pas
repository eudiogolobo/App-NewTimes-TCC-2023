...
{$R *.dfm}
uses
U_principalpas, u_altpesturma,u_inicio,U_principal_professor;

procedure Tfrm_cad_avaliacao.btn_cancelarClick(Sender: TObject);
begin
if MessageDlg('Você perderá tudo que já fez, deseja mesmo cancelar?',mtConfirmation,[mbyes, mbno],0)=mryes then
begin
   ...
   with dm.QMATRICULA do
    ...
    begin
     ...
     begin
       ...
       parambyname('Nota').AsFloat:= 00.00;
       ExecSQL;
     end;
     dm.qmatricula.next;
   end;
...
procedure Tfrm_cad_avaliacao.FormMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure Tfrm_cad_avaliacao.Panel2MouseMove(Sender: TObject;
